...
/*
    @brief Wait for W25Q64 to be busy
    @param None
    @return None
*/  
void W25Q64_WaitBusy(void) // This function checks if W25Q64 is busy before performing any operations, and waits if it is busy
{
    uint8_t res;
    do{
        CS_ENABLE;
        SPI0_TransmitAndReceive(0x05); // Try to read SPI status
        res = SPI0_TransmitAndReceive(0xff); // Send any content to get the status and check if it is busy
        CS_DISENABLE;
    }while((res & 0x01) == 0x01);
}
...
/*
    @brief Erase a 64KB block of W25Q64
...
void W25Q64_Erase64k(uint8_t block_num)
{
    ...
    CS_DISENABLE;
    W25Q64_WaitBusy();// Erasing is slow, so wait a bit longer
}


/*
    @brief Write data to the specified page of W25Q64 Flash chip, starting from page_num, default write 256 bytes (1 page = 256 bytes)
    @param data_buffer Pointer to the start address of the data array to be written
    @param page_number_from_start Start page number to write
    @return None
*/
...
/*
 * Read data from W25Q64 Flash chip
 * @param recv: Pointer to the buffer for receiving data
 * @param read_addr: Flash address to read
...
void W25Q64_ReadData(uint8_t *recv, uint32_t read_addr, uint32_t datalen)
{
    ...
    SPI0_Read(recv, datalen); // Read specified length of data
