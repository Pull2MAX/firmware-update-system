...
{
    fmc_unlock(); // Flash must be unlocked for erasure
    for(uint16_t i = 0; i < erase_num; ++i)
    {
        fmc_page_erase((0x08000000 + (startPage + i) * 1024)); // 0x08000000 is the starting address of flash, plus the page offset to get the actual erase address
    }
    fmc_lock(); // Flash must be locked after erasure
}

/**  
 * @brief       When writing to internal Flash memory, it can only be written in one go of 4 bytes
 *  
 ...
 * @param[in]   write_length    The number of bytes to write, must be a multiple of 4 (e.g., writing 3 words would be 12 bytes)  
 *  
 * @note        Calling specifications:  
 *              1. Ensure the target Flash area has been erased (Flash can only write 1 to 0, and after erasure, it is all 1s)  
...
void GD32_WriteFlash(uint32_t start_addr, uint32_t *write_content, uint32_t write_length)
{
    ...
    while(write_length)
    {
        fmc_word_program(start_addr, *write_content); // This function writes four bytes of data at once
        write_length -= 4;
        start_addr += 4;
        write_content++;
    }
