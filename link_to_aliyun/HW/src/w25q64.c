...
void W25Q64_WaitBusy(void) // This function checks if W25Q64 is busy before performing any operation, if busy, keep waiting
{
    uint8_t res;
    do{
        CS_ENABLE;
        SPI0_TransmitAndReceive(0x05); // Try to read SPI status
        res = SPI0_TransmitAndReceive(0xff); // Send a dummy content to get the status to see if it is busy
        CS_DISENABLE;
    }while((res & 0x01) == 0x01);
}
...
/**
 * @brief Write data to the specified page page_num of the W25Q64 Flash chip, the data address starts from data_buffer, default to write 256 bytes at a time (1 page = 256 bytes) and then finish writing
 * 
 * @param data_buffer Pointer to the start address of the data to be written
...
{
    uint8_t write_data[4];    // Array to store read command and address
