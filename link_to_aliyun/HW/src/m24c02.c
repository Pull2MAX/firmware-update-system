...
{
    IIC_Start();
    IIC_Send_Byte(M24C02_WADDR); // From the perspective of the computer, this is selecting the device and then writing
...
uint8_t M24C02_ReadData(uint8_t addr, uint16_t read_length, uint8_t *read_arr) // The return value of the function represents error information, parameters need to manually input the starting address to read and the content to be read
{
    ...
    IIC_Start(); // According to the timing diagram, it is necessary to re-enter the device address and the host is in read mode
...
{
    memset(&ota_info, 0, OTA_CONTROL_INFO_SIZE); // This is equivalent to initializing the values of ota_info, then the next line is the actual reading and writing to ota_info
