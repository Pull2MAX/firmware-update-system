...
void SPI0_Init(void)
{
    ...
    spi_para.nss = SPI_NSS_SOFT;    // Software control NSS
    spi_para.endian = SPI_ENDIAN_MSB;  // MSB first transmission
    spi_para.clock_polarity_phase = SPI_CK_PL_LOW_PH_1EDGE; // Clock polarity low, sample on first edge
    spi_para.prescale = SPI_PSC_2;  // Prescale 2
...
void SPI0_Read(uint8_t *read_store_arr, uint16_t datalen)
{
    for(uint16_t i = 0; i < datalen; ++i)
    {
        read_store_arr[i] = SPI0_TransmitAndReceive(0xff); // Provide a dummy write content, then wait to receive data and store it in read_store_arr
    }
}
